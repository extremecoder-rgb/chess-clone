...
const renderBoard = () => {
    ...
    board.forEach((row, rowIndex) => {
        row.forEach((square, colIndex) => {
            ...
            // Create and append piece elements if present
            if (square) {
    ...
    // Flip the board based on player role
    if (playerRole === "b") {
...
const handleMove = (source, target) => {
    const move = {
        ...
        promotion: "q" // Promote all pawns to queens for simplicity
    };
    ...
    if (result) {
        ...
        // If the move is invalid, reset the dragged piece
        renderBoard();
    }
};
...
const getPieceUnicode = (type, color) => {
    ...
    return pieceUnicode[type] ? pieceUnicode[type][color] : '';
};
